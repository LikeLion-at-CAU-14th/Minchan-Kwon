...
KAKAO_SCOPE = get_secret("KAKAO_SCOPE")
...
def kakao_login(request):
    return redirect(f"{KAKAO_REDIRECT}?client_id={KAKAO_CLIENT_ID}&response_type=code&redirect_uri={KAKAO_CALLBACK_URI}&scope={KAKAO_SCOPE}")
...
def kakao_callback(request):
    ...
    token_req = requests.post(
        "https://kauth.kakao.com/oauth/token", # 카카오 토큰 서버 주소
        data={
            "grant_type": "authorization_code", # 카카오 문서에 명시된 고정값
            "client_id": KAKAO_CLIENT_ID,
            "client_secret": KAKAO_SECRET,      # 카카오 디벨로퍼스에서 보안 설정 활성화 시 필요
    ...
    user_info_response = requests.get(
        "https://kapi.kakao.com/v2/user/me", # 카카오 사용자 정보 API 주소
        headers={
            "Authorization": f"Bearer {kakao_access_token}",
            "Content-type": "application/x-www-form-urlencoded;charset=utf-8" # 헤더 추가
        },
        timeout=10,
    )
    ...
    if kakao_id is None:
        return JsonResponse({"error": "Failed to get Kakao User ID."}, status=status.HTTP_400_BAD_REQUEST)
